<html>
<head></head>

<body>
</body>
<html>
<head>
  <title>VroomVroomBids.com</title>
  <link rel="stylesheet" type="text/css" href="css/register.css">
</head>
<body class="body">
  <div class="title_wrapper">
    <div class="login_register">
      <form action="profile.php">
        <button class="Login" type="submit"><b>Back</b></button>
      </form>
    </div>
    <h1 class="title">Vroom Vroom Bids</h1>
    <h2 class="under_title">- The best auction site to sell your motor vehicle</h2>
  </div>
  <div class="form_wrapper">
    <form method="POST" action="new_name.php" class="form" enctype="multipart/form-data">
    <b>Name:<input type="text" name="nume" class="input"><br>
      Surname:<input type="text" name="prenume" class="input"><br>
      Password:<input type="password" name="parola_1" class="input"><br>
      <button type="submit" class="button_register"><b>Submit</b></button></b>
    </form>
  </div>
</html>
